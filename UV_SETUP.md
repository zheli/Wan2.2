...
This project ships with a `pyproject.toml`, so you can let the [`uv`](https://github.com/astral-sh/uv) package manager create and maintain a virtual environment for you. The commands below default to creating `.venv/` in the repository root.
...
```bash
uv python install 3.10          # optional; ensures the required Python is available
